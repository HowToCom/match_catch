        ...
        <div className="flex-1 flex flex-col px-6 overflow-y-auto">
          <div className="space-y-4">
            <div>
              ...
              <div className={`flex items-center rounded-2xl px-4 py-3.5 transition-all border-2 ${focusedField === 'userId' ? 'border-primary bg-white shadow-sm shadow-primary/10' : 'border-transparent bg-gray-50'}`}>
            ...
            <div>
              ...
              <div className={`flex items-center rounded-2xl px-4 py-3.5 transition-all border-2 ${focusedField === 'password' ? 'border-primary bg-white shadow-sm shadow-primary/10' : 'border-transparent bg-gray-50'}`}>
          ...
          <div className="mt-8 space-y-3">
            <button
              onClick={handleLogin}
              disabled={isLoading}
              className={`w-full py-4 rounded-2xl text-sm font-bold transition-all ${isLoading ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-primary to-primary-light text-white active:scale-[0.98] shadow-lg shadow-primary/25'}`}
            >
              {isLoading ? (
            ...
            <Link to="/register" className="flex items-center justify-center w-full py-4 rounded-2xl border-2 border-gray-100 text-sm font-bold text-gray-500 hover:border-primary/30 hover:text-primary transition-all">
              회원가입
            </Link>
          </div>
          <p className="text-center text-[11px] text-gray-300 mt-8 pb-8 font-medium">충남대학교 분실물 매칭 서비스</p>
        </div>
      </div>
    </Layout>
  );
