    ...
    >
      <div className="w-full max-w-[412px] bg-white rounded-t-3xl overflow-hidden shadow-2xl"
        ...
        <div className="flex px-5 pt-2 pb-0 gap-2">
          {[{ key: 'date', label: '📅 날짜' }, { key: 'time', label: '⏰ 시간' }].map(t => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${
                tab === t.key ? 'bg-primary text-white shadow-sm' : 'bg-gray-100 text-gray-500'
              }`}
            >
              {t.label}
            </button>
        ...
        {tab === 'date' && (
          <div className="px-5 pt-3 pb-2">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm font-black text-gray-900">{viewYear}년 {viewMonth + 1}월</span>
          ...
          <div className="px-5 pt-3 pb-2">
            <div className="flex gap-3" style={{ height: '200px' }}>
              <div className="flex-1 flex flex-col">
                ...
                <div ref={hourRef} className="flex-1 overflow-y-auto space-y-1 scrollbar-none" style={{ scrollbarWidth: 'none' }}>
                  {hours.map(h => (
                    <button key={h} onClick={() => setSelHour(h)}
                      className={`w-full py-2 rounded-xl text-sm font-bold transition-all active:scale-95 ${selHour === h ? 'bg-primary text-white shadow-sm' : 'text-gray-600'}`}>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div ref={minRef} className="flex-1 overflow-y-auto space-y-1 scrollbar-none" style={{ scrollbarWidth: 'none' }}>
                  {mins.map(m => (
                    <button key={m} onClick={() => setSelMin(m)}
                      className={`w-full py-2 rounded-xl text-sm font-bold transition-all active:scale-95 ${selMin === m ? 'bg-primary text-white shadow-sm' : 'text-gray-600'}`}>
...
function UploadPage() {
  ...
  const isFormValid = () => {
    if (mode === 'found') return selectedFile && formData.content.trim();
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'found') {
        ...
        navigate('/postdetail', {
          state: { postId: response.found_item_id, mode: 'found', isAuthor: true, isFromUpload: true },
        });
      } else {
    ...
    <Layout modal={pickerModal}>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4 bg-gray-50 pb-24">
        <div
          ...
          } ${previewUrl ? 'shadow-md' : 'border-2 border-dashed bg-white shadow-sm border-gray-200'}`}
        >
          {previewUrl ? (
            <div className="relative w-full h-full">
              <img src={previewUrl} alt="선택된 사진" className="w-full h-full object-cover" />
              {!isDisabled && (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
          <div className="divide-y divide-gray-50">
            ...
            <FormRow label={mode === 'found' ? '습득 장소 · 선택' : '분실 예상 장소 · 선택'} focused={focusedField === 'location'}>
              <div className="flex items-start gap-2 mt-1.5">
                ...
                <button type="button" onClick={fetchLocation} disabled={isLocating || isDisabled}
            ...
            <FormRow label={mode === 'found' ? '습득 시간 · 선택' : '분실 예상 시간 · 선택'} focused={focusedField === 'time'}>
              <button type="button" onClick={() => setShowPicker(true)} disabled={isDisabled}
